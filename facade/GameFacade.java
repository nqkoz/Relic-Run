...
if (direction == null) { System.out.println("Précisez une direction."); return; }
Player p = GameState.getInstance().getPlayer();
if (p.move(direction)) {
describeLocation();
} else {
...
Player p = GameState.getInstance().getPlayer();
if (!p.take(itemName)) System.out.println("Aucun objet nommé '"+itemName+"' ici.");
}


public void drop(String itemName) {
Player p = GameState.getInstance().getPlayer();
if (!p.drop(itemName)) System.out.println("Vous n'avez pas '"+itemName+"'.");
}
...
Player p = GameState.getInstance().getPlayer();
if (!p.use(itemName, target)) System.out.println("Utilisation impossible.");
}


public void talk(String npcName) {
if (npcName == null) { System.out.println("À qui parler ?"); return; }
Player p = GameState.getInstance().getPlayer();
NPC npc = p.getCurrentRoom().findNPC(npcName);
if (npc == null) { System.out.println("Personne nommé '"+npcName+"' ici."); return; }
...
if (e == null) { System.out.println("Aucun ennemi '"+enemyName+"' ici."); return; }
...
public void save(String filename) { saveLoad.save(filename); }
public void load(String filename) { saveLoad.load(filename, factory); }


public boolean checkWinCondition() {
World w = GameState.getInstance().getWorld();
Player p = w.getPlayer();
// Gagne si l'artefact ancien est dans l'inventaire ET joueur de retour à la salle de départ
